...
{
    public class BlockGenerator : Block, IMechanicalPowerBlock
    {
        ...
        public override bool DoPlaceBlock(IWorldAccessor world, IPlayer byPlayer, BlockSelection blockSel, ItemStack byItemStack)
        {
            ...
            if (base.DoPlaceBlock(world, byPlayer, blockSel, byItemStack) && world.BlockAccessor.GetBlockEntity(blockSel.Position) is BlockEntityGenerator blockEntityGenerator)
            {
                blockEntityGenerator.Facing = facing;
                BlockFacing val = FacingHelper.Directions(blockEntityGenerator.Facing).First();
        ...
        public override void OnNeighbourBlockChange(IWorldAccessor world, BlockPos pos, BlockPos neibpos)
        {
            ...
            if (world.BlockAccessor.GetBlockEntity(pos) is BlockEntityGenerator blockEntityGenerator)
            {
                BlockFacing val = FacingHelper.Faces(blockEntityGenerator.Facing).First();
        ...
        {
            if (world.BlockAccessor.GetBlockEntity(pos) is BlockEntityGenerator blockEntityGenerator && blockEntityGenerator.Facing != 0)
            {
                return FacingHelper.Directions(blockEntityGenerator.Facing).First() == face;
            }
            return false;
        }
        ...
        public override void OnJsonTesselation(ref MeshData sourceMesh, ref int[] lightRgbsByCorner, BlockPos pos, Block[] chunkExtBlocks, int extIndex3d)
        {
            ...
            if (val == null || !(this.api.World.BlockAccessor.GetBlockEntity(pos) is BlockEntityGenerator blockEntityGenerator) || blockEntityGenerator.Facing == Facing.None)
            {
                return;
            }
            Facing facing = blockEntityGenerator.Facing;
            if (!MeshData.TryGetValue(facing, out var value))
            {
                Vec3f val2 = new Vec3f(0.5f, 0.5f, 0.5f);
                Block block = ((IWorldAccessor)val.World).GetBlock(new AssetLocation("electricity:generator-stator"));
